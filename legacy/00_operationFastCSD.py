...
def performFastCSD(mqmin,mqmax,npts_csd):
    ...
    nsteps = npts_csd
    if Ilow>250.0: Ilow = 240.0
    if Ihigh>250.0: Ihigh = 250.0

    with open(directory+'/csd_'+tnowstr,'w') as outfile:
        timesteps, ipoints, batmanfield, faradaycup = get_csd(Ilow,Ihigh,nsteps,wasin,tallstart)           
...
while again:
    ...
    if request_csd:
        ...
        if request_csd == 2:
            ...
            performFastCSD(venus.read(['csd_MQ_min']),venus.read(['csd_MQ_max']),venus.read(['num_csd_points']))
    ...
    if venus.read(['peaking_request']):
        ...
        ##  Eventtually get rid of the next lines.  Just using this to diagnose how peaking is working
        with open('peaking_results','a') as fff:
            fff.write("%i %6.3f %7.3f %.5f %7.3f %.5f\n"%(int(tpeaking),tlastave-tpeaking,ibatman_pre_request,bbatman_pre_request,
                                                          ibatmanrequest,bbatmanrequest))
