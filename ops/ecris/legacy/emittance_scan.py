...
class Variables:
    ...
    def write_and_save_file(self):
        ...
        file_name = (
            "Emittance_Scanner_Variables_" + Date_Time + ".txt"
        )  # create a filename including the date and time, so that each new file that is written and saved has a unique name. Can be used to next time load variables.
        variables = {
            "Maximal x' [mrad]": self.xp_max,
            "Minimal x' [mrad]": self.xp_min,
            "Maximal y' [mrad]": self.yp_max,
            "Minimal y' [mrad]": self.yp_min,
            "Maximal x [mm]": self.x_max,
            "Minimal x [mm]": self.y_min,
            "Maximal y [mm]": self.y_max,
            "Minimal y [mm]": self.y_min,
            "Charge Number Q": self.Q,
            "Mass Number M": self.M,
            "Extraction Voltage U [V]": self.V_extr,
            "x' Step Size [mrad]": self.xp_step,
            "y' Step Size [mrad]": self.yp_step,
            "x Step Size [mm]": self.x_step,
            "y Step Size [mm]": self.y_step,
        }
    ...
    def open_and_read_file(self, file_name):
        ...
        if re.match(
            pattern, file_name
        ):  # check if the file that we are trying to read has the correct file name
...
class Motor:
    def __init__(self, beam_line=None):
        ...
        self.centered = [
            ...
            False,
            False,
        ]  # list to confirm if centering has been done (i.e. if 0 position has been redefined)
        self.mid_point_offsets = [
            30.18,
            36.50,
            31.75,
            31.75,
        ]  # Midpoint Offsets, i.e. the difference between the In/positive Limit and true 0 position [mm]
        # midpoint offsets for VENUS from LabView program
        self.tn = telnetlib.Telnet(
            "10.10.100.60", 5002, timeout=3
        )  # opens connection to controller
        self.send_command(
            "PROG0"
        )  # opens Program0 prompt. Because Master = 0 all commands have to be sent in Prog0
        self.send_command(
            "ACC 5 DEC 5 VEL 15 STP 100"
        )  # sets Acceleration Ramp, Decceleration Ramp, Velocity and Stop Ramp
    ...
    def move_to(
        self, position, axis
    ):  # positon in units (depends on what the acr is calibrated to); axis = 0,1,2,3
        """
        ...
        if not self.axis_clear(axis):
            self.move_to(
                200, [1, 0, 3, 2][axis]
            )  # make big enough move, so that motor will travel to positive EOT limit switch
            self.send_command(
                f"CLR BIT({8467 + axis * 32})"
            )  # clear kill all moves (hitting limit switch sets kill all moves request)
    ...
    def send_command(self, command, Print=False):
        ...
        time.sleep(
            0.07
        )  # need to await response from controller. If the delay here is too small we might not catch the enire response which will lead to errors in reading position or other relevant information
    ...
    def axis_clear(self, axis):
        ...
        other_axis = [1, 0, 3, 2][
            axis
        ]  # if axis = 0, then other axis = 1, if axis = 2, other axis = 3 and vice versa
...
class Read_and_Analyze:
    def __init__(self, Variables_instance, Motor_instance):
        ...
        self.x = np.arange(
            self.Var.x_min, self.Var.x_max + self.Var.x_step, self.Var.x_step
        )  # x-array
        self.y = np.arange(
            self.Var.y_min, self.Var.y_step + self.Var.y_max, self.Var.y_step
        )  # y-array
        self.x_prime = np.arange(
            self.Var.xp_min, self.Var.xp_step + self.Var.xp_max, self.Var.xp_step
        )  # x' array
        self.y_prime = np.arange(
            self.Var.yp_min, self.Var.yp_step + self.Var.yp_max, self.Var.yp_step
        )  # y' array
    ...
    def get_current(self, axis):
        ...
        V = (
            self.Var.get_V(momentum * 1e-3) / 100
        ...
        for i in position:
            ...
            for j in V:
                ...
                ljm.eWriteName(
                    handle, output, j + 3.188
                )  # according to measurements from Powersupply Voltagedrop is approx. 3.188V
                time.sleep(delay)  #!!!delay for some time so that signal can reach capacitor
                current = 0
                for k in range(2000):  # take 2000 samples
                    current += (
                        ljm.eReadName(handle, Input) / self.Mot.Voltagecurrentfactor
                    )  # actually reading voltage that depends on current
                    time.sleep(
                        0.02
        ...
        variables = {
            ...
            "Minimal x' [mrad]": self.Var.xp_min,
            "Minimal y' [mrad]": self.Var.yp_min,
            "Maximal y' [mrad]": self.Var.yp_max,
            "Maximal x [mm]": self.Var.x_max,
            "Minimal x [mm]": self.Var.x_min,
            "Maximal y [mm]": self.Var.y_max,
            "Minimal y [mm]": self.Var.y_min,
            "Charge Number Q": self.Var.Q,
            "Mass Number M": self.Var.M,
            "Extraction Voltage U [V]": self.Var.V_extr,
            "x' Step Size [mrad]": self.Var.xp_step,
            "y' Step Size [mrad]": self.Var.yp_step,
            "x Step Size [mm]": self.Var.x_step,
        ...
        file_name = (
            ...
            + Date_Time
            + f" {['Venus_x_', 'Venus_y_', 'AECR_x_', 'AECR_y_'][axis]}"
            + ".txt"
        )
    ...
    def emittance(self, axis, I):
        ...
        sigma_position_squared = sum((I @ (position - position_mean) ** 2)) / np.sum(
            I
        )  # sqrt(sum((x_i-<x>)^2*I_i)\sum()
        sigma_momentum_squared = sum((I.T @ (momentum - momentum_mean) ** 2)) / np.sum(I)
        sigma_position_sigma_momentum = (
            (momentum - momentum_mean).T @ I @ (position - position_mean) / np.sum(I)
        )
        E_rms = np.sqrt(
            sigma_position_squared * sigma_momentum_squared - (sigma_position_sigma_momentum) ** 2
        )  # RMS Emittance. Actual Emittance is 4* RMS Emittance
    ...
    def phase_space_plot(self, filename):
        ...
        )  # parametrisizing the ellipse (Epsilon = 4*Epsilon_rms)
        img_filename = (
...
def main():
    ...
    file_bool = bool(
        int(
            input("To load existing variables from a file enter 1. To set new variables enter 0: ")
        )
    )
    if file_bool:
        while True:
            Varfile = str(
                input(
                    "Enter file name (i.e. 'Emittance_Scanner_Variables_mm-dd-yyyy HHhMMmSSs.txt'): "
                )
            )
            try:
                ...
                print(
                    "This looks like the wrong file. Check if the file has the format 'Emittance_Scanner_Variables_mm-dd-yyyy HHhMMmSSs.txt'"
                )
        ...
        for i, (var_label, var_name, value) in enumerate(variables_dict):
            if var_name in ["Q", "M"]:
                ...
                if V.get_V(value * 1e-3) > 200:
                    value = round(
                        200 / V.d / V.V_extr * V.L / 2 * 1e3, 3
                    )  # if momentum too big, set to maximum
                    print(var_label + " is too big. The Maximum is set to " + str(value))
                setattr(
                    V, var_name, max(0, value)
                )  # set variable, but can't be negative --> max(0,value)
                ...
                while True:
                    try:
                        ...
                        if value > 2 * getattr(
                            V,
                            variables_dict[np.where(variables_dict == var_name)[0].item() - 1][1],
                        ):
                ...
                while True:
                    value = input(
                        "Enter " + var_label + " (Entering Nothing sets the minimum to -maximum): "
                    )
                    if value == "":
                        setattr(
                            V,
                            ...
                            -getattr(
                                V,
                                variables_dict[
                                    np.where(variables_dict == var_name)[0] - 2, [1]
                                ].item(),
                            ),
                        )
                        break
                        ...
                        try:
                            ...
                            if var_name in ["xp_min", "yp_min"] and V.get_V(value * 1e-3) < -200:
                                ...
                                print(
                                    var_label + "is too small. The Minimum is set to " + str(value)
                                )
                                setattr(V, var_name, value)
                            else:
                                value = min(
                                    0, value
                                )  # x_min, y_min, xp_min, yp_min is negative --> min(0,value)
                                setattr(V, var_name, value)
                            break
                        except ValueError:
                            print(var_label + " must be a float!")
        ...
        while True:
            try:
                save = np.where(
                    np.array(["no", "yes"])
                    == input("Do you want to save those variables? (yes or no): ")
                )[0].item()
                if save:
    ...
    if plot_data:
        while True:
            data_file = input(
                "Enter the file name (i.e. Emittance_Scanner_Data_{beam_line}_{axis}_mm-dd-yyyy HHhMMmSSs.txt : "
            )
            try:
                data = []
                ...
                while True:
                    try:
                        again = np.where(
                            np.array(["no", "yes"])
                            == input(
                                "Uncompatible file! Do you want to enter a new file? (yes or no) (if 'no', the program is closed)"
                            )
                        )[0].item()
                        break
                    except ValueError:
    ...
    while True:
        try:
            M.beam_line = np.where(
                np.array(["VENUS", "AECR"]) == input("Enter the Beam Line (VENUS or AECR): ")
            )[0].item()
    ...
    while True:
        try:
            M.Voltagecurrentfactor = float(
                input("Enter the Current to Voltage Gain factor [1e3, 1e4, ..., 1e11] [V/A]: ")
            )
